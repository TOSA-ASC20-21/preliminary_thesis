...
readheadercmd = '/ASC/presto-master/bin/readfile %s' % filename
print readheadercmd
...
try:
    ...
    for line in ddplan:
        ...
        for i, dml in enumerate(dmlist):
            ...
            subDM = np.mean(dml)
            if maskfile:
                prepsubband = "/ASC/presto-master/bin/prepsubband -sub -subdm %.2f -nsub %d -downsamp %d -mask ../%s -o %s %s" % (subDM, Nsub, subdownsamp, maskfile, rootname, '../'+filename)
            else:
                prepsubband = "/ASC/presto-master/bin/prepsubband -sub -subdm %.2f -nsub %d -downsamp %d -o %s %s" % (subDM, Nsub, subdownsamp, rootname, '../'+filename)
            print prepsubband
            ...
            prepsubcmd = "/ASC/presto-master/bin/prepsubband -nsub %(Nsub)d -lodm %(lowdm)f -dmstep %(dDM)f -numdms %(NDMs)d -numout %(Nout)d -downsamp %(DownSamp)d -o %(root)s %(subfile)s" % {
...
try:
    ...
    for df in datfiles:
        fftcmd = "/ASC/presto-master/bin/realfft %s" % df
        print fftcmd
    ...
    for fftf in fftfiles:
        searchcmd = "/ASC/presto-master/bin/accelsearch -zmax %d %s"  % (zmax, fftf)
        print searchcmd
...
try:
    ...
    for cand in cands:
        ...
        foldcmd = "/ASC/presto-master/bin/prepfold -n %(Nint)d -nsub %(Nsub)d -dm %(dm)f -p %(period)f %(filfile)s -o %(outfile)s -noxwin -nodmsearch" % {
